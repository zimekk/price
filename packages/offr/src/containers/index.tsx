  ...
  list.reduce(
    (meta, { type, data, created }) =>
      Object.assign(
        ...
        // { type },
        data.offers &&
          data.offers.price <= meta.minPrice && {
            minPrice: data.offers.price,
            minPriceChanged: new Date(created).getTime(),
          },
        data.offers &&
          data.offers.price > meta.maxPrice && {
            maxPrice: data.offers.price,
          },
        data.offers &&
          0 === meta.price && {
...
function Summary({
  ...
  const fetcher = () =>
    fetch(`/api/offr/history?item=${item}`)
      .then(async (res) =>
  ...
  return (
    <div>
      <div style={{ float: "right", fontSize: "small" }}>
        #
        <Link
          href={data.offers ? data.offers.url : "#"}
          onClick={(e) => {
      ...
      {data.offers && data.offers.availability && (
        <>
          &nbsp;
          <span
            style={{
              ...
              color:
                {
                  InStock: "limegreen",
                  OutOfStock: "orangered",
                }[data.offers.availability] || "black",
    ...
    <div style={{ borderTop: "1px solid lightgray", marginTop: ".25em" }}>
      ...
      <strong>
        {meta.minPriceChanged === dayjs(created).valueOf() ? (
          ...
          data.offers &&
          meta.minPrice === data.offers.price && (
            <i
              style={{
                position: "absolute",
                marginLeft: "-1em",
...
export function Price() {
  ...
  const fetcher = () =>
    ...
    fetch(`/api/offr?limit=${queries.limit}`))
      .then(async (res) =>
        res.ok ? await res.json() : Promise.reject(await res.text()),
      )
